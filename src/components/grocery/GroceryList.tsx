...
export default function GroceryList() {
  ...
  // Gate on meals.length > 0 so users who open Grocery before adding meals don't
  // flip the autoGenerated flag on a doomed POST (it 400s with no meals) and
  // then never retry once meals arrive.
  useEffect(() => {
    if (!loading && !list && meals.length > 0 && !generating && !autoGenerated) {
      setAutoGenerated(true);
      handleAutoGenerate();
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [loading, list, meals.length]);
